 ...
 *   not an Error.
 */
function _toGoogleError(err, outcome) {
    if (err instanceof googleError_1.GoogleError) {
        return err;
    }
    if (!(err instanceof Error)) {
        return err;
    }
    const error = new googleError_1.GoogleError(err.message);
    error.cause = err;
    ...
    const httpStatus = typeof fetchError.status === 'number'
        ? fetchError.status
        : fetchError.response?.status;
    if (typeof httpStatus === 'number') {
        error.code = (0, status_1.rpcCodeFromHttpStatusCode)(httpStatus);
    ...
    if (outcome.timedOut) {
        const timedOutError = new googleError_1.GoogleError(`Deadline exceeded: ${outcome.rpcName} did not respond within ${outcome.timeoutMs} milliseconds.`, { cause: err });
        timedOutError.code = status_1.Status.DEADLINE_EXCEEDED;
    ...
    error.code = status_1.Status.UNAVAILABLE;
    return error;
}
function generateServiceStub(rpcs, protocol, servicePath, servicePort, auth, requestEncoder, responseDecoder, numericEnums, minifyJson) {
    ...
    for (const [rpcName, rpc] of Object.entries(rpcs)) {
        ...
        serviceStub[rpcName] = (request, metadata, callOptions, callback) => {
            metadata ??= {};
            ...
            if (callOptions?.deadline && !rpc.responseStream) {
                ...
                const deadlineMs = callOptions.deadline instanceof Date
            ...
            let timedOut = false;
            const requestSignals = [cancelSignal];
            ...
            if (rpc.responseStream) {
                streamArrayParser.cancelSignal.addEventListener('abort', () => (cancelRequested = true), { once: true });
            ...
            auth
                ...
                if (response.ok && rpc.responseStream) {
                    ...
                    return Promise.all([
                        ...
                        .then(([ok, buffer]) => {
                        const response = responseDecoder(rpc, ok, buffer, httpStatusCode);
